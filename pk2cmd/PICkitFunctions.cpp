...
bool CPICkitFunctions::SetVDDVoltage(float voltage, float threshold)
{
    if (voltage < 1.7F)
    {
        voltage = 1.7F;  // minimum, as when forcing VDD Target can get set very low (last reading)
    ...
    if (writeUSB(commandArray, 1))
    {
        if (readUSB())
        {
            if (m_Driver.type() == Pickit2 || m_Driver.type() == PK2M) {
                ...
                *vpp = (float)((Usb_read_array[1] * 256) + Usb_read_array[2]) * 0.125f;
                *vdd = (float)((Usb_read_array[3] * 256) + Usb_read_array[4]) * 0.125f;
                return true;
            }
        }
    }
...
bool CPICkitFunctions::SearchDevice(int familyIndex)
        {
            float searchVdd = vddSetPoint;
            ActiveFamily = familyIndex;
			// set VDD voltage:
            if (_tcsncmp(DevFile.Families[ActiveFamily].FamilyName, "EEPROMS/SPIFLASH 1v8", 20) == 0)
            {
                searchVdd = 1.8;                             // Use 1.8V search voltage for SPIFLASH 1v8 family
                SetVDDVoltage(searchVdd, (float)0.85);
                Sleep(50);                                  // Allow time for voltage to settle if previous setting was higher
            }
            else
            {
                SetVDDVoltage(searchVdd, (float)0.85);    // Other families can be searched with default (3v3) voltage
            }
